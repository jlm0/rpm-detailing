...
export default async function LandingFooterWrapper() {
  ...
  const footerData = {
    ...
    description: siteSettings?.description || "Your trusted partner for premium car detailing services. We restore and protect your vehicle's beauty with meticulous care.",
    ...
    ctaText: siteSettings?.footerCTA?.text || "Ready for a showroom shine? Book your car detailing appointment today!",
    ctaButtonText: siteSettings?.footerCTA?.buttonText || "Book Now",
    ctaButtonLink: siteSettings?.footerCTA?.buttonLink || "/booking",
    copyright: siteSettings?.copyright?.replace('{year}', new Date().getFullYear().toString()) || `© ${new Date().getFullYear()} RPM Detailing. All Rights Reserved.`,
  };
